...
assert.doesNotMatch(activeRendered.at(-1), /data-analysis-artifact="mindmap"/);
assert.doesNotMatch(activeRendered.at(-1), /data-analysis-artifact="path"/);
assert.match(activeRendered.at(-1), /Åpne separat Tankekart-forhåndsvisning/);
assert.match(activeRendered.at(-1), /Åpne separat Sti-forhåndsvisning/);
