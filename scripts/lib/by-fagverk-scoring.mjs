...
export function scoreBy(textValue, corpus, policy) {
  ...
  const rules = policy.chapter_rules || {};
  const cfg = policy.scoring || {};

  const scores = corpus.entries.map((entry) => {
    const rule = rules[entry.chapter_id] || {};
    const matched = [];
    let score = 0;
    for (const candidate of chapterTerms(entry, rule)) {
      ...
      const multiplier = globalNonScoring.has(candidate.term) ? 0 : termPolicy ? Number(termPolicy.multiplier || 0) : 1;
  ...
  const minScore = Number(cfg.grounded_min_score || 6);
  const minTerms = Number(cfg.grounded_min_terms || 2);
  const margin = Number(cfg.ambiguity_margin || 3);
  let status = "unsupported";
  if (top && top.score >= minScore && top.matched_terms.length >= minTerms) {
    status = second && second.score >= minScore && (top.score - second.score) < margin ? "ambiguous" : "grounded";
  }
