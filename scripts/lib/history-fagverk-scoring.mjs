...
function temporalEvidence(text, tokens, temporalGate) {
  const terms = (temporalGate?.terms || []).map(normalize).filter(Boolean);
  const matchedTerms = terms.filter((term) => termPresent(text, tokens, term));
  const matchedYears = [...new Set(text.match(/\b(?:1[0-9]{3}|20[0-9]{2})\b/g) || [])];
  return {
    matched_terms: matchedTerms,
    matched_years: matchedYears,
    eligible: temporalGate?.required === false || matchedTerms.length > 0 || matchedYears.length > 0
  };
}

export function scoreHistory(textValue, corpus, policy) {
  ...
  const temporal = temporalEvidence(text, tokens, policy.temporal_gate || { required: true, terms: [] });
  ...
  const scores = corpus.entries.map((entry) => {
    ...
    const eligible = temporal.eligible && anchorEligible;
    ...
    return {
      ...
      eligible,
      eligibility_reason: !temporal.eligible ? "missing_temporal_anchor" : anchorEligible ? "eligible" : "missing_required_anchor",
