...
assert.doesNotMatch(html, /src="chat\.html\?ahaSemanticModelShadow=1&amp;ahaInsightSynthesisV2=1"/);
...
assert.match(operator, /FRAME_URL = "chat\.html\?ahaSemanticModelShadow=1&ahaInsightSynthesisV2=1"/);
assert.match(operator, /URLSearchParams/);
assert.match(operator, /frame\.src = FRAME_URL/);
const intentGuardIndex = operator.indexOf('if (operatorIntent !== OPERATOR_INTENT)');
const frameNavigationIndex = operator.indexOf('frame.src = FRAME_URL');
const frameLoadHandlerIndex = operator.indexOf('frame.addEventListener("load"');
assert.ok(intentGuardIndex >= 0 && frameLoadHandlerIndex > intentGuardIndex && frameNavigationIndex > frameLoadHandlerIndex,
  "Chat navigation must happen only after exact intent guard and after load handler registration");
