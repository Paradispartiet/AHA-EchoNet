...
assert.equal(header.includes('class="chat-profile-card"'), false, 'profile access should live in the shared footer, not the Chat header');
...
assert.match(fs.readFileSync('js/ahaGlobalNav.js', 'utf8'), /id="aha-global-footer-profile"/, 'Chat should inherit the global profile action');
