...
assert.ok(providerLoaderAt > uiRuntimeAt && providerLoaderAt < runtimeFacadeAt, 'provider loader must load after providers and before the runtime facade');
assert.ok(runtimeFacadeAt > providerLoaderAt && runtimeFacadeAt < chatAt, 'runtime facade must load after provider loader and before ahaChat.js');
