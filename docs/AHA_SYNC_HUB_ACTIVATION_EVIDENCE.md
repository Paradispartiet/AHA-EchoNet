...
| **D. Dry-run/preview gate** | `js/ahaManualSyncDryRunTargetAdapter.js`; `js/ahaDashboard.js`; `js/ahaManualSyncAdapter.js`; `tests/aha-manual-sync-dry-run-target-adapter.test.cjs`; `tests/aha-sync-hub-go-no-go-blockers.test.cjs`; `tests/aha-manual-sync-run-summary-preview.test.cjs` | Det nye target-adapterlaget er preview-only/no-write. Det teller aktive records og begge tombstone-former, inspiserer runtime/sync-funksjon uten å kalle den og returnerer en blokkert samlet plan med tom `wouldRun`, `wouldWrite: false`, `wouldCallSyncFromDatabase: false` og `wouldCallRepository: false`. | **GO for preview-only; activation NO-GO** | Preview-UI mangler fortsatt. Execution forblir NO-GO til alle andre gates er GO og en separat activation-PR finnes. |
...
Dry-run target-adapteren og no-write-testdekningen er nå på plass, men den aktive Home-rendereren har ingen previewflate. Activation skal fortsatt ikke skje fordi de samlede gates A–J ikke er GO. Neste anbefalte PR er derfor:

```text
feat: show manual sync dry-run target preview
```

Denne PR-en skal kun vise den eksisterende blokkerte no-write-planen i en tydelig previewflate. Den skal ikke bygge en kjørbar sync-knapp, laste modulruntime, aktivere execution eller skrive data.
