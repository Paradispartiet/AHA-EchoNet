...
assert.ok(providerLoaderAt > uiRuntimeAt && providerLoaderAt < capabilityBindingsAt, 'provider loader must load after providers and before capability bindings');
assert.ok(capabilityBindingsAt > providerLoaderAt && capabilityBindingsAt < runtimeFacadeAt, 'capability bindings must load before the runtime facade');
assert.ok(runtimeFacadeAt > capabilityBindingsAt && runtimeFacadeAt < chatAt, 'runtime facade must load after capability bindings and before ahaChat.js');
assert.ok(runtimeCompositionAt > runtimeFacadeAt && runtimeCompositionAt < chatAt, 'runtime composition must load after its providers and before ahaChat.js');
assert.ok(applicationCompositionAt > runtimeCompositionAt && applicationCompositionAt < chatAt, 'application composition must load after runtime composition and before ahaChat.js');
...
const chatOrchestratorSource = fs.readFileSync('js/ahaChatApplicationComposition.js', 'utf8');
const chatBootstrapSource = fs.readFileSync('js/ahaChat.js', 'utf8');
...
assert.doesNotMatch(chatOrchestratorSource, /function (?:getLatestAhaReplyFromDom|ensureAcademicAfterworkShape|getAhaExportDeps|buildAhaAnalysisExportBundle|formatAhaAnalysisExportMarkdown|copyAhaAnalysisExportMarkdown|exportAhaAnalysisJson)\s*\(/, 'bound export runtime ownership must remain extracted');
assert.doesNotMatch(chatOrchestratorSource, /function (?:cleanArticleText|toSentences|collectOpinionArticleEvidence|detectTextType|normalizeSubjectLinks|enrichSubjectMatchesForClimateConflict|detectPublicAdministrationReformSignal|detectPublicAdministrationSignal|enrichSubjectMatchesForPublicAdministration|normalizeFagkoblinger|inferReligiousLexiconEvidence|isAcademicLikeType|isDayLogType)\s*\(/, 'text, signal and subject module calls must remain direct bindings');
assert.doesNotMatch(chatOrchestratorSource, /function (?:analysisTopicMismatch|safeMarkupText|safeMarkupList|safeMarkupSortItems|buildHistoryGoSuggestion|filterCrossDomainAutoPayload|buildOpinionArticleQualityAnalysis|normalizeAhaVisibleReply)\s*\(/, 'auto-output policy and direct analysis/reply bindings must remain extracted');
assert.doesNotMatch(chatOrchestratorSource, /function (?:getThemeId|getFieldId|out|setStatusNote|escHtml|renderAuxPanel|renderPanel|currentInsights|normalizeDisplayText)\s*\(/, 'chat shell DOM and active-insight ownership must remain extracted');
assert.doesNotMatch(chatOrchestratorSource, /function (?:resolveConceptTerm|suggestCategoryChips|refreshAhaExplorer|renderAhaChatMemoryStatus|normalizeHistoryGoLinks)\s*\(/, 'chat shell adapters and canonical History Go normalization must remain extracted');
assert.doesNotMatch(chatOrchestratorSource, /global\.(?:AHATestHooks|AHAMemoryControls|AHAMemoryDebug|AHAMemoryExclusions|AHAActiveRun|AHAChat)\s*=/, 'public globals and test hooks must remain extracted');
assert.doesNotMatch(chatOrchestratorSource, /document\.addEventListener\("DOMContentLoaded"/, 'DOM bootstrap must remain extracted');
...
assert.match(chatBootstrapSource, /providerLoader\.instantiate\("applicationComposition", \{/);
assert.doesNotMatch(chatBootstrapSource, /providerLoader\.(?:require|instantiate)\("(?:agentRuntime|ingestRuntime|runtimeComposition)"/, 'bootstrap must not own the provider graph');
