...
import { scoreNature } from "./lib/nature-fagverk-scoring.mjs";
...
const PATHS = {
  ...
  output: "data/evaluation/aha-nature-fixture-correction-report.v1.json"
};
...
function buildReport(corpus, policy, corrections) {
  ...
  if (corpus.subject_filter !== "natur" || policy.subject_id !== "natur" || corrections.subject_id !== "natur") {
    validationErrors.push("Fixture correction inputs are not consistently Nature-scoped.");
  }
  ...
  if (!Array.isArray(corrections.cases) || corrections.cases.length !== 16) {
    validationErrors.push("Nature fixture correction matrix must contain all 16 canonical fixtures.");
  }

  const cases = (corrections.cases || []).map((correction) => {
    const fixture = readJson(correction.fixture_path);
    const result = scoreNature(fixture.inputText, corpus, policy);
    const errors = [];
    if (result.status !== correction.expected_nature_status) {
      errors.push(`Expected status ${correction.expected_nature_status}, got ${result.status}.`);
    }
    ...
    return {
      ...
      errors,
      expected_nature_status: correction.expected_nature_status,
  ...
  return {
    schema: "aha_nature_fixture_correction_report_v1",
    version: "1.0.0",
...
function main() {
  ...
  if (args.help) {
    console.log("Usage: node scripts/compare-nature-fixture-corrections.mjs [--corpus path] [--policy path] [--corrections path] [--output path]");
    return;
  }
  const report = buildReport(readJson(args.corpus), readJson(args.policy), readJson(args.corrections));
  writeJson(args.output, report);
  console.log(`Nature fixture corrections: ${report.summary.passed}/${report.summary.total} passed.`);
