...
const DEFAULTS = {
  corpus: "data/integrations/candidates/history-go-fagverk-musikk.candidate.v1.json",
  audit: "data/integrations/candidates/history-go-fagverk-musikk.candidate-audit.v1.json",
  config: "data/integrations/review/history-go-fagverk-musikk.review-config.v1.json",
  matrix: "data/evaluation/aha-musikk-fagverk-evaluation-matrix.v1.json",
  fixtures: "data/evaluation/aha-politics-fixture-corrections.v1.json",
  policyOutput: "/tmp/aha-musikk-policy.json",
  evaluationOutput: "/tmp/aha-musikk-evaluation.json",
  fixturesOutput: "/tmp/aha-musikk-fixtures.json"
};
...
const entryTerms = (entry) => [...(entry.title_terms || []), ...(entry.concept_terms || []), ...(entry.support_terms || [])].map(normalize).filter(Boolean);
...
function subjectOverlap() {
  ...
  for (const file of fs.readdirSync(dir).filter((name) => /^history-go-fagverk-.+\.candidate\.v1\.json$/.test(name))) {
    ...
    for (const term of new Set(corpus.entries.flatMap(entryTerms))) {
      if (!byTerm.has(term)) byTerm.set(term, new Set());
      byTerm.get(term).add(subject);
    }
  }
  return byTerm;
}
...
function validate(corpus, audit, config) {
  if (corpus.subject_filter !== "musikk") throw new Error("Candidate is not Musikk-scoped.");
  if (JSON.stringify(audit.subject_filter) !== JSON.stringify(["musikk"])) throw new Error("Audit is not Musikk-scoped.");
  if (config.subject_id !== "musikk") throw new Error("Config is not Musikk-scoped.");
  if (corpus.source_ref !== audit.source_ref || corpus.source_ref !== config.source_ref) throw new Error("Musikk source_ref mismatch.");
  if (corpus.content_sha256 !== config.corpus_sha256) throw new Error("Musikk corpus digest mismatch.");
  if (audit.gate?.passed !== true || (audit.gate?.errors || []).length) throw new Error("Musikk candidate audit gate failed.");
  if (audit.coverage?.expected !== 8 || audit.coverage?.registered !== 8 || audit.coverage?.materialized !== 8) throw new Error("Musikk coverage is not 8/8/8.");
  if (corpus.entries.length !== 8 || corpus.entries.some((entry) => (entry.module_source_paths || []).length !== 3)) throw new Error("Musikk 8-chapter/24-module contract failed.");
  ...
  if (configured.size !== chapters.size || [...chapters].some((id) => !configured.has(id))) throw new Error("Musikk chapter rules do not exactly cover candidate.");
  const candidateTerms = new Set(corpus.entries.flatMap(entryTerms));
  for (const [chapterId, rule] of Object.entries(config.chapter_rules || {})) {
    for (const anchor of rule.required_anchor_terms || []) {
      if (!candidateTerms.has(normalize(anchor))) throw new Error(`${chapterId}: anchor is not an exact candidate term: ${anchor}`);
    }
  }
  if (config.approval_required !== true || config.activation_allowed !== false || config.runtime_activation_allowed !== false) throw new Error("Musikk lifecycle guard invalid.");
}

function buildPolicy(corpus, audit, config) {
  validate(corpus, audit, config);
  const overlaps = subjectOverlap();
  ...
  const terms = [];
  for (const term of [...new Set(corpus.entries.flatMap(entryTerms))].sort((a, b) => a.localeCompare(b, "nb"))) {
    const collision = collisions.get(term);
    const crossSubjects = [...(overlaps.get(term) || new Set())].sort();
    ...
    else if (contextOnly.has(term)) { action = "context_only"; category = "music_context_only"; multiplier = 0; }
    ...
    if (action) terms.push({term, action, multiplier, category, internal_risk: collision?.risk || null, chapter_count: collision?.chapter_count || 1, chapters: collision?.chapters || [], cross_subject_count: crossSubjects.length, cross_subjects: crossSubjects});
  }
  const summary = terms.reduce((acc, item) => {
    acc.total += 1; acc[item.action] += 1; acc.categories[item.category] = (acc.categories[item.category] || 0) + 1; return acc;
  }, {total:0, non_scoring:0, down_weight:0, context_only:0, categories:{}});
  return {
    schema:"aha_musikk_fagverk_term_policy_v1", version:"1.0.0", status:config.status,
    source_repo:corpus.source_repo, source_ref:corpus.source_ref, registry_version:corpus.registry_version,
    corpus_sha256:corpus.content_sha256, subject_id:"musikk", lifecycle_stage:config.lifecycle_stage,
    ...
    chapter_rules:config.chapter_rules, summary, terms,
...
function score(textValue, corpus, policy) {
  ...
  const scores = corpus.entries.map((entry) => {
    ...
    return {chapter_id:entry.chapter_id,title:entry.title,score:Number(total.toFixed(3)),eligible,eligibility_reason:eligible?"eligible":"missing_required_anchor",matched_anchor_terms:matchedAnchors,matched_terms:matched};
  });
...
function evaluate(corpus, policy, matrix) {
  ...
  for (const testCase of matrix.positive_cases || []) {
    ...
    const result=score(testCase.text,corpus,policy), errors=[];
  ...
  for (const testCase of matrix.abstention_cases || []) {
    const result=score(testCase.text,corpus,policy);
...
function compareFixtures(corpus, policy, baseline) {
  ...
  for (const reviewCase of baseline.cases || []) {
    ...
    if (result.status!=="unsupported") errors.push(`Musikk must abstain; got ${result.status}${result.selected_chapter_id?` (${result.selected_chapter_id})`:""}.`);
    results.push({id:reviewCase.id,fixture_path:reviewCase.fixture_path,expected_musikk_status:"unsupported",source_evidence:evidence,result,passed:errors.length===0,errors});
  }
...
function main() {
  ...
  const policy=buildPolicy(corpus,audit,config), evaluation=evaluate(corpus,policy,matrix), fixtures=compareFixtures(corpus,policy,baseline);
  writeJson(args.policyOutput,policy); writeJson(args.evaluationOutput,evaluation); writeJson(args.fixturesOutput,fixtures);
  console.log(`Musikk review: ${policy.chapters.length} chapters; ${policy.summary.total} classified terms; evaluation ${evaluation.summary.passed}/${evaluation.summary.total}; fixtures ${fixtures.summary.passed}/${fixtures.summary.total}.`);
