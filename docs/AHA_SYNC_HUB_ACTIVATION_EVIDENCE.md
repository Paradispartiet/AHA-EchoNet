...
| **D. Dry-run/preview gate** | `index.html`; `js/ahaManualSyncDryRunTargetAdapter.js`; `js/ahaDashboard.js`; `js/ahaManualSyncAdapter.js`; `tests/aha-home-manual-sync-dry-run-preview.test.cjs`; `tests/aha-manual-sync-dry-run-target-adapter.test.cjs`; `tests/aha-sync-hub-go-no-go-blockers.test.cjs`; `tests/aha-manual-sync-run-summary-preview.test.cjs` | Target-adapterlaget er preview-only/no-write, og Home viser nå den blokkerte planen i eksisterende Sync Hub diagnostics/expanded UI. Previewen viser planstatus, blockers, targets, lokale counts, tombstones og runtime-/funksjonsstatus. Den kaller ikke sync-funksjonen og planen har fortsatt tom `wouldRun`, `wouldWrite: false`, `wouldCallSyncFromDatabase: false` og `wouldCallRepository: false`. | **GO for UI preview-only; activation NO-GO** | Actual execution forblir NO-GO til alle andre gates er GO og en separat activation-PR finnes. UI-preview er ikke activation evidence for execution. |
...
Den blokkerte dry-run target-planen vises nå i aktiv Home Sync Hub med trygg fallback og uten execution, writes eller modulruntime-loading. Gate D har dermed UI-evidence for preview-only, men actual manual execution er fortsatt **NO-GO**, og activation-PR-en er fortsatt ikke tillatt. Neste anbefalte PR er derfor:

```text
test: lock AHA manual sync dry-run target evidence
```

Denne PR-en skal bare styrke samlet evidence for preview-, fallback- og no-write-kontrakten. Den skal ikke aktivere manual sync, auto-sync, database-/repository-kall eller laste Lists-, Paths-, Groups- eller AHAavisa-runtime på Home.
