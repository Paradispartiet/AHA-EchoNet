(function (global) {
  ...
  function chapterEmne(raw, subject) {
    ...
    return { emne_id: `fagverk_${subject.subject_id}_${id}`, title: String(item.title || id), core_concepts: unique(item.core_concepts || []), keywords: unique(item.keywords || []), thinkers: unique(item.thinkers || []), methods: unique(item.methods || []), chapter_specific_terms: unique(item.semantic_terms || []), learning_goals: [], checkpoints: [], summary: String(item.subtitle || ""), description: String(item.subtitle || ""), fagverk: { source_repo: "Paradispartiet/History-Go", source_ref: String(item.source_ref || subject.source_ref || ""), canonical_subject_id: subject.subject_id, chapter_id: id, source_path: String(item.source_path || ""), registry_path: "data/fagverk/fagverk_registry.json", manifest_path: "data/fag/fag_manifest.json", package_field: "chapter_registry", minimum_matched_terms: 2, term_source: "history_go_registry_plus_manifest_emner_v2", generation_mode: "canonical_history_go_deployment_index_v2" } };
  }
