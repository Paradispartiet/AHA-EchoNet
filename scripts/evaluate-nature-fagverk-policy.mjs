...
import { normalize, scoreNature } from "./lib/nature-fagverk-scoring.mjs";

const here = path.dirname(fileURLToPath(import.meta.url));
const repoRoot = path.resolve(here, "..");
const PATHS = {
  corpus: "data/integrations/candidates/history-go-fagverk-natur.candidate.v1.json",
  ...
  output: "data/evaluation/aha-nature-fagverk-evaluation-report.v1.json"
};
...
function evaluateCase(testCase, corpus, policy, kind) {
  const result = scoreNature(testCase.text, corpus, policy);
  const errors = [];
  if (kind === "ambiguity") {
...
function validateInputs(corpus, policy, matrix) {
  const errors = [];
  if (corpus.subject_filter !== "natur") errors.push("Corpus is not Nature-scoped.");
  if (policy.subject_id !== "natur") errors.push("Policy is not Nature-scoped.");
...
function buildReport(corpus, policy, matrix) {
  ...
  return {
    schema: "aha_nature_fagverk_evaluation_report_v1",
    version: "1.0.0",
...
function main() {
  ...
  if (args.help) {
    console.log("Usage: node scripts/evaluate-nature-fagverk-policy.mjs [--corpus path] [--policy path] [--matrix path] [--output path]");
    return;
  }
  const report = buildReport(readJson(args.corpus), readJson(args.policy), readJson(args.matrix));
  writeJson(args.output, report);
  console.log(`Nature evaluation: ${report.summary.passed}/${report.summary.total} passed; ${report.summary.chapters_covered} chapters covered.`);
