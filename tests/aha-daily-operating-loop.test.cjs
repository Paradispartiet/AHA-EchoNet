...
c.AHAChatPersistence.buildChatIntakeCandidates=()=>undefined;
c.AHAChatPersistence.collectChatStats=()=>undefined;
status=c.AHADailyOperatingLoop.buildDailyLoopStatus({save:false,now:'2026-07-07T08:00:00.000Z'});
assert.equal(status.meta.unscannedChatCandidates,0,'Daily Loop must tolerate unavailable optional Chat candidate data');
