...
import { normalize, scoreHistory } from "./lib/history-fagverk-scoring.mjs";
...
const PATHS = {
  ...
  output: "data/evaluation/aha-history-fagverk-evaluation-report.v1.json"
};
...
function evaluateCase(testCase, corpus, policy, kind) {
  const result = scoreHistory(testCase.text, corpus, policy);
  const errors = [];
  if (kind === "ambiguity") {
...
function validateEvidence(matrix, corpus, policy) {
  ...
  for (const testCase of [...matrix.positive_cases, ...matrix.confusion_cases]) {
    ...
    for (const evidence of testCase.required_evidence || []) {
      ...
      if (!terms.has(term)) errors.push(`${testCase.id}: required evidence not found in expected chapter or its review rule: ${term}.`);
      const termPolicy = policyByTerm.get(term);
      if (globalNonScoring.has(term) || termPolicy?.action === "non_scoring" || termPolicy?.action === "context_only") {
  ...
  for (const testCase of matrix.ambiguity_cases) {
    for (const evidence of testCase.non_scoring_evidence || []) {
      const term = normalize(evidence);
      const termPolicy = policyByTerm.get(term);
      if ((!termPolicy || !["non_scoring", "context_only"].includes(termPolicy.action)) && !globalNonScoring.has(term)) {
        errors.push(`${testCase.id}: negative evidence must be explicitly non-scoring: ${evidence}.`);
      }
    }
  }
  return errors;
...
function validateInputs(corpus, policy, matrix) {
  const errors = [];
  if (corpus.subject_filter !== "historie") errors.push("Corpus is not History-scoped.");
  if (policy.subject_id !== "historie") errors.push("Policy is not History-scoped.");
  if (matrix.subject_id !== "historie") errors.push("Matrix is not History-scoped.");
  ...
  if (corpus.entries.length !== 23) errors.push("History corpus does not contain 23 chapters.");
  if (matrix.positive_cases.length !== 23) errors.push("Evaluation matrix must contain one positive case per chapter.");
  return errors;
}

function buildReport(corpus, policy, matrix) {
  const evidenceErrors = [...validateInputs(corpus, policy, matrix), ...validateEvidence(matrix, corpus, policy)];
  const cases = [
  ...
  return {
    schema: "aha_history_fagverk_evaluation_report_v1",
    version: "1.0.0",
...
function main() {
  ...
  if (args.help) {
    console.log("Usage: node scripts/evaluate-history-fagverk-policy.mjs [--corpus path] [--policy path] [--matrix path] [--output path]");
    return;
  }
  const report = buildReport(readJson(args.corpus), readJson(args.policy), readJson(args.matrix));
  writeJson(args.output, report);
  console.log(`History evaluation: ${report.summary.passed}/${report.summary.total} passed; ${report.summary.chapters_covered} chapters covered.`);
