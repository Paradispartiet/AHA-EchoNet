...
(function (global) {
  ...
  function buildDailyOperatingLoopPackSafe(){ try { const api=global.AHADailyOperatingLoop; if(!api) return { available:false, status:"missing", headline:"Dagens AHA-løype mangler.", currentFocus:"", nextBestAction:null, suggestedPrompts:[], changedSinceLastRun:null }; const d=api.buildDailyLoopStatus?.({lightweight:true,save:false}) || api.loadDailyLoopStatus?.() || {}; return { available:true, status:asText(d.status), headline:asText(d.headline), currentFocus:asText(d.currentFocus), nextBestAction:d.nextBestAction||null, suggestedPrompts:asArray(d.suggestedPrompts).slice(0,6), changedSinceLastRun:d.changedSinceLastRun||null }; } catch { return { available:false, status:"error", headline:"Daily Operating Loop unavailable.", currentFocus:"", nextBestAction:null, suggestedPrompts:[], changedSinceLastRun:null }; } }
  ...
  function buildAgentContext(profile, options = {}) {
    ...
    const dailyOperatingLoopPack = options.dailyOperatingLoopPack && typeof options.dailyOperatingLoopPack === "object" ? options.dailyOperatingLoopPack : buildDailyOperatingLoopPackSafe();
    if (dailyOperatingLoopPack) context.dailyOperatingLoopPack = dailyOperatingLoopPack;
  ...
  function createChatSession(deps = {}) {
    const updateEmptyState = typeof deps.updateEmptyState === "function" ? deps.updateEmptyState : () => {};
    const setStatusNote = typeof deps.setStatusNote === "function" ? deps.setStatusNote : () => {};
    ...
    function startMetaAiSession(payload) {
      ...
      renderMetaAiSessionBox(activeSession);
    ...
    function renderMetaAiClaimCard(parent, claim) {
      ...
      const buttonRow = global.document.createElement("div");
    ...
    function maybeHandleMetaAiAgentReply(rawReply) {
      ...
      let parsed = null;
      try {
        parsed = parseAgentResponse(rawReply);
      } catch {
        return null;
      }
    ...
    return {
      getActiveMetaAiSession,
      renderMetaAiSessionBox,
      startMetaAiSession,
      saveMetaAiClaimFeedback,
      renderMetaAiClaims,
