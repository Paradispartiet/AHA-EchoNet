...
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.canonical_main_sha, "3e7ddcf895739e0d0e8d4042b8b5fc6736b95185");
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.diagnostic_pull_request, 970);
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.diagnostic_disposition, "closed_without_merge");
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.diagnostic_workflow_run, 35378726636);
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.diagnostic_node_run_number, 2014);
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.release_gate_passed, true);
assert.equal(audit.current_human_review_projection.current_reprojection_evidence.product_coverage.lists.qualified_case_share, 0.818182);
...
assert.equal(humanReview.archived_live_review_input.admissible_for_human_scoring, true);
assert.equal(humanReview.archived_live_review_input.blocking_reason, null);
assert.equal(humanReview.archived_live_review_input.current_reprojection_evidence.canonical_main_sha, "3e7ddcf895739e0d0e8d4042b8b5fc6736b95185");
assert.equal(humanReview.archived_live_review_input.current_reprojection_evidence.diagnostic_pull_request, 970);
assert.equal(humanReview.archived_live_review_input.current_reprojection_evidence.diagnostic_disposition, "closed_without_merge");
assert.equal(humanReview.archived_live_review_input.current_reprojection_evidence.diagnostic_workflow_run, 35378726636);
assert.equal(humanReview.archived_live_review_input.current_reprojection_evidence.diagnostic_node_run_number, 2014);
assert.deepEqual(humanReview.archived_live_review_input.current_reprojection_evidence.product_coverage, {
  lists: { qualified_case_count: 18, qualified_case_share: 0.818182, passed: true },
  paths: { qualified_case_count: 18, qualified_case_share: 0.818182, passed: true },
  mindmap: { qualified_case_count: 18, qualified_case_share: 0.818182, passed: true }
});
...
assert.equal(humanReview.archived_live_review_input.current_reprojection_evidence.release_gate_passed, true);
...
assert.equal(humanReview.review_workflow.human_scoring_input_ready, true);
assert.equal(humanReview.review_workflow.next_required_evidence, "independent_human_review_scores_and_attestation");
...
assert.ok(humanReview.case_reviews.every((entry) => entry.lists == null && entry.paths == null && entry.mindmap == null), "governance sync must not fabricate human scores");
assert.equal(humanReview.status, "agent_pre_review_complete_independent_human_review_open");
