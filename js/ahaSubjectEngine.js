(function (global) {
  ...
  function unique(values) {
    const seen = new Set();
    return (Array.isArray(values) ? values : [values]).flatMap((value) => Array.isArray(value) ? value : [value]).map((value) => String(value || "").trim()).filter((value) => {
      const key = normalize(value); if (!key || seen.has(key)) return false; seen.add(key); return true;
    });
  }
  ...
  async function overlayIndex() {
    ...
    try {
      ...
      console.warn("AHASubjectEngine: overlays unavailable", error);
      cache.overlays = [];
    }
    return cache.overlays;
  }

  function canonicalTargets(entry) {
    return unique(Array.isArray(entry?.canonical_subject_ids) && entry.canonical_subject_ids.length ? entry.canonical_subject_ids : [entry?.subject_id]);
  }
      ...
      try {
        ...
        for (const emne of Array.isArray(data?.emner) ? data.emner : []) {
          ...
          result.push({
            ...emne,
            local_knowledge: {
              ...object(emne.local_knowledge),
              classification: String(emne?.local_knowledge?.classification || "aha_overlay"),
              canonical_subject_ids: unique(emne?.local_knowledge?.canonical_subject_ids || [subjectId]),
              revalidate_on_runtime_change: true
            }
          });
        }
      } catch (error) {
        console.warn(`AHASubjectEngine: overlay ${file} unavailable`, error);
      }
    }
    return result;
  }
  ...
  function canonicalEmne(raw, subject) {
    ...
    return {
      ...
      fagverk: {
        ...
        source_ref: String(item.source_ref || subject.source_ref || ""),
        canonical_subject_id: subject.subject_id,
        source_path: String(item.source_path || subject?.package?.emner_path || ""),
        registry_path: "data/fagverk/fagverk_registry.json",
        manifest_path: "data/fag/fag_manifest.json",
        package_field: "emner",
        minimum_matched_terms: 2,
        term_source: "history_go_manifest_emner_v2",
  ...
  function methodEmne(raw, subject) {
    const item = object(raw);
    return {
      emne_id: `method:${String(item.method_id || "")}`,
      title: String(item.title || item.short_label || item.method_id || ""),
      core_concepts: unique([item.short_label, ...(item.data_forms || [])]),
      keywords: unique(item.emne_affinities || []),
      thinkers: [], methods: [], learning_goals: [], checkpoints: [],
      summary: String(item.description || ""), description: String(item.description || ""),
      fagverk: {
        source_repo: "Paradispartiet/History-Go",
        source_ref: String(item.source_ref || subject.source_ref || ""),
        canonical_subject_id: subject.subject_id,
        source_path: String(item.source_path || subject?.package?.methods_path || ""),
        registry_path: "data/fagverk/fagverk_registry.json",
        manifest_path: "data/fag/fag_manifest.json",
        package_field: "methods",
        minimum_matched_terms: 2,
        term_source: "history_go_manifest_methods_v2",
        generation_mode: "canonical_history_go_deployment_index_v2"
      }
    };
  }
  ...
  async function loadPackage(subjectId, field) {
    const index = await deploymentIndex();
    const subject = index.subjects.find((item) => item.subject_id === subjectId);
  ...
  async function loadSubject(subjectId) {
    ...
    const index = await deploymentIndex();
    const canonical = index.subjects.find((item) => item.subject_id === id);
    if (!canonical) return null;
    const overlays = await localEmner(id);
    const canonicalEntries = [
      ...(canonical.emner || []).map((item) => canonicalEmne(item, canonical)),
      ...(canonical.methods || []).map((item) => methodEmne(item, canonical)),
      ...(canonical.chapters || []).map((item) => chapterEmne(item, canonical))
    ];
    const seen = new Set();
    const emner = [...canonicalEntries, ...overlays].filter((entry) => {
      const key = `${entry.emne_id}|${normalize(entry.title)}`;
      if (!entry.emne_id || seen.has(key)) return false;
      seen.add(key); return true;
    });
    const subject = {
      subject_id: canonical.subject_id,
      subject_label: canonical.subject_label,
      description: canonical.description,
      kind: canonical.kind,
      parent_subject_id: canonical.parent_subject_id || null,
      schema_family: canonical.schema_family || "",
      source_ref: canonical.source_ref,
      authority: "history_go_canonical_fagverk",
      emner
    };
    cache.subjects[id] = subject;
    return subject;
  }

  async function loadAllSubjects() {
    return (await Promise.all((await listSubjects()).map((subject) => loadSubject(subject.subject_id)))).filter(Boolean);
  }
  ...
  function buildMatchProvenance(subject, emne) {
    if (emne?.fagverk) return {
      kind: "canonical_fagverk",
      evidence_role: "reference_support_not_source_evidence",
      canonical_subject_id: String(emne.fagverk.canonical_subject_id || subject.subject_id),
      chapter_id: String(emne.fagverk.chapter_id || ""),
      source_repo: String(emne.fagverk.source_repo || "Paradispartiet/History-Go"),
      source_ref: String(emne.fagverk.source_ref || subject.source_ref || ""),
      source_path: String(emne.fagverk.source_path || ""),
      registry_path: String(emne.fagverk.registry_path || ""),
      manifest_path: String(emne.fagverk.manifest_path || ""),
      package_field: String(emne.fagverk.package_field || ""),
      term_source: String(emne.fagverk.term_source || ""),
      generation_mode: String(emne.fagverk.generation_mode || "")
    };
    if (emne?.local_knowledge) return {
      kind: "aha_local_overlay",
      evidence_role: "local_reference_support_not_source_evidence",
      classification: String(emne.local_knowledge.classification || ""),
      canonical_subject_ids: unique(emne.local_knowledge.canonical_subject_ids || []),
      revalidate_on_runtime_change: true
    };
    return null;
  }
  ...
  async function matchText(text, options = {}) {
    ...
    if (!target) return [];
    let subjects;
    try { subjects = await loadAllSubjects(); }
