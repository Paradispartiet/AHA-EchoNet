...
(function () {
  ...
  function buildProductStatus(options = {}) {
    ...
    const dailyLoop = window.AHADailyOperatingLoop?.buildDailyLoopStatus?.({ lightweight: true, save: false }) || window.AHADailyOperatingLoop?.loadDailyLoopStatus?.() || null;
    const status = {
      ...
      dailyOperatingLoop: { available: Boolean(window.AHADailyOperatingLoop), status: dailyLoop?.status || "unavailable", href: navigation.homeHref + "#aha-local-home-daily-loop", headline: dailyLoop?.headline || "Dagens AHA-løype", currentFocus: dailyLoop?.currentFocus || "Start med Home.", nextBestAction: dailyLoop?.nextBestAction || null, actionQueueCount: (dailyLoop?.actionQueue || []).length, changedSinceLastRun: dailyLoop?.changedSinceLastRun || null },
