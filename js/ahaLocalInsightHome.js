...
(function (global) {
  ...
  function buildPendingWork(payload) { const c = safeObject(payload.counts); return [ { id:"intake", label:"Data Intake review", count:num(c.intakeReview), href:"intake.html" }, { id:"curation", label:"Curation review", count:num(c.curationReview), href:"curation.html" }, { id:"graph", label:"Graph insights", count:num(c.graphInsights), href:"knowledge-map.html#graph-intelligence" }, { id:"training", label:"Training ready", count:num(c.trainingReady), href:"training.html" }, { id:"workflow", label:"Workflow audit", count:c.workflowScore ? 1 : 0, status:c.workflowScore ? `${c.workflowScore}/100` : "mangler", href:"knowledge-workbench.html" } ]; }
  ...
  function buildModuleTiles(payload) { const c = safeObject(payload.counts); const s = safeObject(payload.sources || {}); return [
    { id:"chat", title:"Chat", href:"chat.html", status:c.chatMessages?"active":"empty", metrics:[`${c.chatMessages} meldinger`, `${safeArray(s.chat?.sessions).length} sessions`, `${safeArray(s.evaluations).length} evalueringer`] },
    ...
    { id:"intake", title:"Data Intake", href:"intake.html", status:c.intakeReview?"needs_review":"active", metrics:[`${num(c.intakeTotal)} total`, `${c.intakeReview} review`, `${num(s.intake?.counts?.approvedCount||s.intake?.approvedCount)} approved`] },
    { id:"curation", title:"Curation", href:"curation.html", status:c.curationReview?"needs_review":"active", metrics:[`${c.curationReview} review`, `${num(s.curation?.counts?.["high"+"Priority"]||s.curation?.["high"+"Priority"])} høy`, `${c.trainingReady} training ready`] },
    { id:"map", title:"Knowledge Map", href:"knowledge-map.html", status:c.mapNodes?"active":"empty", metrics:[`${c.mapNodes} nodes`, `${c.mapEdges} edges`, `${c.activeProjects} prosjekter`, `${c.topConcepts} begreper`] },
    { id:"graph", title:"Graph Intelligence", href:"knowledge-map.html#graph-intelligence", status:c.graphInsights?"active":"starting", metrics:[`${c.graphInsights} insights`, `${num(s.graph?.counts?.suggestedLinks||s.graph?.suggestedLinks)} links`, `${num(s.graph?.counts?.trainingOpportunities||s.graph?.trainingOpportunities)} muligheter`] },
    { id:"training", title:"Training", href:"training.html", status:c.corpusItems?"active":"starting", metrics:[`${c.corpusItems} corpus`, `${num(s.examples?.counts?.total||s.examples?.total)} examples`, `${c.trainingReady} ready`] },
  ...
  global.AHALocalInsightHome = { VERSION, STORAGE_KEY, buildHomeInsightPayload, collectHomeCounts, buildHomeHeadline, buildPrimaryAction, buildHighlights, buildActiveProjects, buildPendingWork, buildRecentActivity, buildModuleTiles, saveHomePayload, loadHomePayload, refreshHome, buildLocalInsightHome, normalizeLocalInsightHomeInput, buildHomeSections, buildHomeDisplay, buildHomeSafety };
