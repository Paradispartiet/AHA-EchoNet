...
async function flushAsyncShadow() {
...
async function run() {
  {
    ...
    assert.equal(shadow.concepts.length, 0);
    assert.equal(shadow.candidate_insights.length, 0, "Phase 1B skal fortsatt ikke kopiere dagens candidates inn i SemanticDocument");
...

  {
    ...
    assert.equal(canonicalCalls.length, 1, "shadow-feil skal ikke stoppe dagens canonical ingest");
