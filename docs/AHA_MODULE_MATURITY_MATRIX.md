...
| insta | AHA Insta | personal | active | 1 | `insta.html` | `js/ahaInsta.js` | `aha_insta_posts_v1`, `aha_insta_stories_v1`, `aha_insta_import_sessions_v1`, `aha_insta_import_preview_v1`, `aha_insta_profile_v1`, likes/comments/follows keys | yes | yes | yes | yes | yes | yes | partial | Local-only boundary tightened; remaining gap is larger social/import model and optional database strategy. | Only consider ready after explicit tests cover no auto-db-sync, no external sharing, local-only metadata, import-preview consent, ingest and tombstone search. |
...
| privacy | Personvern | system | active | 1 | `privacy.html` | `js/ahaPrivacy.js` | `aha_privacy_settings_v1`, AHA data report keys | no | no | no | no | yes | yes | ready | Privacy viser og eksporterer lokale AHA-nøkler inkludert Feed, Gallery og Insta-underlag, teller local-only/import/tombstone/sync/EchoNet/publiseringsflagg, og har tester for rapport, eksport og eksplisitt sletting. | Utvid datarapporten eksplisitt når nye moduler legger til nye localStorage-nøkler; ikke legg inn skjult auto-discovery som kan blande inn uvedkommende data. |
