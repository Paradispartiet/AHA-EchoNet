...

{
  ...
  assert.equal(runtime.getLastEvaluation().metrics.evidence_fidelity_rate, 1);
...

{
  ...
  const result = runtime.handleModelShadow({ source_event_id: "wrong_source", source_text_hash: data.hash });
...

{
  ...
  const result = runtime.handleModelShadow({ source_event_id: data.deterministic.source_event_id, source_text_hash: data.hash });
  assert.equal(result, null);
  assert.equal(gateCalls.length, 0);
}
...

{
  ...
  const invalidEvaluation = {
    schema: "aha_semantic_evaluation_v1",
    version: 1,
    source_event_id: base.deterministic.source_event_id,
    source_text_hash: base.hash,
    valid: false,
  ...
  const { runtime, data, events } = makeRuntime({ evaluation: invalidEvaluation });
  const result = runtime.handleModelShadow({
    source_event_id: data.deterministic.source_event_id,
    source_text_hash: data.hash
  });
  assert.ok(result);
  assert.equal(result.valid, false, "invalid evaluation skal fortsatt være observerbar i shadow runtime");
  assert.equal(result.gate.synthesis_allowed, false);
...

{
  ...
  assert.equal(runtime.bind(), false);
