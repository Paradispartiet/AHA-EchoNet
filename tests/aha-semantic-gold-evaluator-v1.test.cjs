...
assert.ok(fixtureSet.fixtures.length >= 2, "gold-settet skal inneholde både perfekt og imperfekt regresjonsfixture");

for (const fixture of fixtureSet.fixtures) {
  const result = api.evaluateGoldFixture({
  ...
  assert.equal(result.production_gate_authority, false, "offline gold-metrikk skal ikke åpne produksjonsport");
...

{
  ...
  fixture.model_shadow.entities.push({
    source_surface: "Oslo",
    canonical_label: "Oslo",
    entity_type: "place"
  });
