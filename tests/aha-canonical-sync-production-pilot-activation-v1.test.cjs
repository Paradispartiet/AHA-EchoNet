...
assert.match(source.dbRunner, /runtime_control_membership_shape\(\)/);
assert.match(source.dbRunner, /control_membership_baseline/);
assert.match(source.dbRunner, /control_membership_after/);
...
assert.match(source.dbRunner, /grant aha_canonical_production_runtime to current_user with inherit false, set true/i);
assert.match(source.dbRunner, /set role aha_canonical_production_runtime/i);
assert.match(source.dbRunner, /pg_terminate_backend\(pid, 5000\)/);
assert.match(source.dbRunner, /reset role/i);
assert.match(source.dbRunner, /revoke aha_canonical_production_runtime from current_user/i);
assert.match(source.dbRunner, /changed the control-plane runtime membership baseline/i);
assert.match(source.dbRunner, /CUT_OFF_NOLOGIN_ZERO_SESSIONS_BASELINE_MEMBERSHIP_RESTORED/);
