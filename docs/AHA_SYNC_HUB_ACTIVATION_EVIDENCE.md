...
|---|---|---|---|---|
| **A. Runtime target gate** | `js/ahaSyncHub.js`; `js/ahaManualSyncAdapter.js`; `docs/AHA_MANUAL_SYNC_TARGET_CONTRACT.md`; `tests/aha-sync-hub-runtime-adapter.test.cjs`; `tests/aha-manual-sync-database-target.test.cjs` | Lists, Paths, Groups og AHAavisa er kartlagt til lokale keys, tabeller og repository-metoder. Dry-run target finnes. Read-only registry bruker samtidig `avisa`, mens manual-sync-laget bruker `ahaavisa`. | **PARTIAL / activation NO-GO** | Én kanonisk, testlåst registry må bevise entydig modul-id, localStorage-key, tabell, target og write-metode for hver modul. |
...
| **D. Dry-run/preview gate** | `js/ahaDashboard.js`; `js/ahaManualSyncAdapter.js`; `tests/aha-manual-sync-database-target.test.cjs`; `tests/aha-manual-sync-run-summary-preview.test.cjs`; `tests/aha-manual-sync-activation-blockers.test.cjs`; `tests/aha-manual-sync-end-to-end-verification.test.cjs` | Target preview, payload preview, checklist, blockers og run summary finnes. Adapter-dry-run rapporterer `wouldExecute: false` og `wouldWrite: false`, og blocker-testen verifiserer null repository writes. | **GO for preview-only; activation NO-GO** | Den endelige brukerreisen må vise target, moduler, record-antall og blockers før kjøring og være koblet til den kanoniske target-registryen uten hidden writes. |
...
| **F. Per-module error gate** | `js/ahaManualSyncAdapter.js`; modulenes repository-persistence-tester; `tests/aha-manual-sync-end-to-end-verification.test.cjs` | Modultester beskytter lokale data ved remote/load-feil. Manual-adapteren har `rollbackStatus: not_available`; en kastet write-feil blir run-level failure fremfor en ferdig dokumentert per-modul continuation. | **NO-GO** | Strukturert resultat per modul, fortsettelse etter én modulfeil, retry-policy og rollback/kompensasjon må dokumenteres og testes. `localStorage` må aldri slettes ved remote-feil. |
...
10. Den samlede activation-testsuiten mangler fortsatt full dekning av alle forbudte triggere, sideeffektklasser og gate-caser.
...
Dry-run/target-adapter og testdekning finnes allerede, men activation skal fortsatt ikke skje fordi de samlede gates A–J ikke er GO. Neste anbefalte PR er derfor:

```text
docs: prepare manual sync activation checklist review
```

Denne PR-en skal fortsatt være dokumentasjon/review. Den skal lukke eller eksplisitt videreføre hvert checklist-punkt mot evidence uten å aktivere execution, laste ny modulruntime eller skrive data.
