...
function validateSubjectCompatibility(candidate, observed, subjectBaseline) {
  const approved = subjectBaseline.subjects?.naeringsliv;
  const observedSubject = observed.subjects?.naeringsliv;
  if (!approved || !observedSubject) throw new Error("Business subject compatibility evidence is missing.");
  if (candidate.source_ref !== approved.approved_source_ref) {
    throw new Error("Business candidate differs from the approved subject-content baseline source.");
  }
  if (observedSubject.content_sha256 !== approved.subject_content_sha256) {
    throw new Error("Observed Business content changed and requires a new subject review.");
  }
}
...
function main() {
  ...
  if (args.help) {
    console.log("Usage: node scripts/build-business-fagverk-expansion-review.mjs [--baseline path] [--candidate path] [--observed path] [--subject-baseline path] [--output path]");
    return;
  }
