const assert = require('assert');
const fs = require('fs');
const vm = require('vm');

const DASHBOARD_FILE = 'js/ahaDashboard.js';
const ADAPTER_FILE = 'js/ahaManualSyncDryRunTargetAdapter.js';
const HOME_FILE = 'index.html';
const HOME_SYNC_MODULES = [
  'js/ahaLists.js',
  'js/ahaPaths.js',
  'js/ahaGroups.js',
  'js/ahaAvisa.js'
];

function read(file) {
  return fs.readFileSync(file, 'utf8');
}

function extractFunction(source, name) {
  const start = source.indexOf(`function ${name}(`);
  assert.notEqual(start, -1, `${name} should exist`);
  const bodyStart = source.indexOf('{', start);
  let depth = 0;
  for (let index = bodyStart; index < source.length; index += 1) {
    if (source[index] === '{') depth += 1;
    if (source[index] === '}') {
      depth -= 1;
      if (depth === 0) return source.slice(start, index + 1);
    }
  }
  assert.fail(`could not extract ${name}`);
}

const dashboardCode = read(DASHBOARD_FILE);
const adapterCode = read(ADAPTER_FILE);
const homeCode = read(HOME_FILE);
const previewRenderer = extractFunction(dashboardCode, 'renderAhaManualSyncDryRunTargetPreview');
const syncHubRenderer = extractFunction(dashboardCode, 'renderSyncHubStatus');

assert.match(previewRenderer, /AHAManualSyncDryRunTargetAdapter/);
assert.match(previewRenderer, /createManualSyncDryRunPlan\s*\(\s*\)/);
assert.match(syncHubRenderer, /renderAhaManualSyncDryRunTargetPreview\s*\(\s*\)/);

for (const label of [
  'Per-module result preview',
  'Preview only',
  'No write',
  'Execution blocked',
  'Manual sync is NO-GO',
  'Auto-sync permanently forbidden'
]) {
  assert.ok(previewRenderer.includes(label), `preview should include ${label}`);
}

for (const field of [
  'localActive',
  'localTombstones',
  'localTotal',
  'runtimeLoaded',
  'syncFunctionAvailable',
  'executionAllowed',
  'blocked',
  'previewStatus',
  'wouldRun',
  'wouldWrite',
  'resultPreview'
]) {
  assert.ok(previewRenderer.includes(field), `preview should use ${field}`);
}

assert.match(previewRenderer, /Per-module preview unavailable: dry-run adapter not loaded/);
assert.match(previewRenderer, /Per-module preview unavailable/);
assert.match(previewRenderer, /No dry-run targets available/);

const context = vm.createContext({
  window: {
    localStorage: {
      getItem(key) {
        const records = {
          aha_lists_v1: [{ id: 'list-active' }, { id: 'list-deleted', deleted_at: '2026-06-01' }],
          aha_paths_v1: [{ id: 'path-active' }],
          aha_groups_v1: [],
          aha_articles_v1: [{ id: 'article-active' }]
        };
        return JSON.stringify(records[key] || []);
      }
    }
  },
  console,
  JSON,
  Object,
  Array,
  Boolean,
  String,
  Set
});
vm.runInContext(adapterCode, context, { filename: ADAPTER_FILE });
vm.runInContext(`
  function escapeHtml(value) {
    return String(value ?? '').replace(/[&<>"']/g, (character) => ({
      '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;'
    })[character]);
  }
  ${extractFunction(dashboardCode, 'formatAhaSyncPreviewValue')}
  ${previewRenderer}
  window.__renderPerModulePreview = renderAhaManualSyncDryRunTargetPreview;
`, context, { filename: DASHBOARD_FILE });

const html = context.window.__renderPerModulePreview();
for (const [targetId, label] of [['lists', 'Lister'], ['paths', 'Stier'], ['groups', 'Grupper'], ['avisa', 'AHAavisa']]) {
  assert.match(html, new RegExp(`>${targetId}<`), `preview should show ${targetId}`);
  assert.match(html, new RegExp(`>${label}<`), `preview should show ${label}`);
}
assert.match(html, />runtime_missing</, 'unloaded Home runtimes should have a safe preview status');
assert.match(html, />0<\/dd>/, 'global preview should report zero targets would run');
assert.match(html, />false<\/dd>/, 'preview should report blocked/no-write boolean values');

const forbiddenPreviewCalls = [
  [/syncFromDatabase\s*\(/, 'syncFromDatabase'],
  [/AHARepository\s*\.\s*(?:save|load)/, 'AHARepository save/load'],
  [/\bsupabase\s*\.\s*from\s*\(/i, 'Supabase'],
  [/\bfetch\s*\(/, 'fetch'],
  [/localStorage\s*\.\s*(?:setItem|removeItem)\s*\(/, 'localStorage write'],
  [/dispatchEvent\s*\(\s*new\s+CustomEvent/, 'source event'],
  [/\bsource_events\b/, 'source_events'],
  [/\bcreateInsight\b/, 'insight creation'],
  [/\bpublish\s*\(/, 'publish']
];
for (const [pattern, label] of forbiddenPreviewCalls) {
  assert.equal(pattern.test(previewRenderer), false, `preview renderer must not contain ${label}`);
  assert.equal(pattern.test(syncHubRenderer), false, `Sync Hub renderer must not contain ${label}`);
}

assert.equal(/<button\b/i.test(previewRenderer), false, 'preview must not expose a sync button');
assert.equal(/(?:execute|run)(?:Handler|ManualSync|Sync)\s*[:=(]/i.test(previewRenderer), false, 'preview must not expose an execute/run handler');
for (const moduleFile of HOME_SYNC_MODULES) {
  assert.equal(homeCode.includes(moduleFile), false, `Home must not load ${moduleFile}`);
}

console.log('aha-manual-sync-per-module-result-preview.test.cjs passed');
