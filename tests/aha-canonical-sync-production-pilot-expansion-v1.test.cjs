...
const verifyPilotExpansionBlock = shellFunctionBlock(source.dbRunner, "verify_pilot_expansion", "add_pilot_profile");
const addPilotProfileBlock = shellFunctionBlock(source.dbRunner, "add_pilot_profile", "deactivate_pilot");
assert.match(verifyPilotExpansionBlock, /<<'SQL'/);
assert.match(addPilotProfileBlock, /<<'SQL'/);
assert.doesNotMatch(verifyPilotExpansionBlock, /-c\s+/);
assert.doesNotMatch(addPilotProfileBlock, /-c\s+/);
assert.match(verifyPilotExpansionBlock, /where id=:'pilot_profile_id'/);
assert.match(addPilotProfileBlock, /where id=:'pilot_profile_id'/);
