...
(function (global) {
  ...
  function buildSemanticIndex(options = {}) {
    ...
    return { generatedAt: nowIso(options), version: VERSION, vectorModel: VECTOR_MODEL, items, stats: { total: items.length, bySource, bySourceType, vectorModel: VECTOR_MODEL, corpusItems: bySource.training_corpus || 0, examples: bySource.training_examples || 0, memoryClaims: bySource.meta_insights_memory || 0 } };
  }
  ...
  function searchSemanticKnowledge(query, options = {}) {
    ...
    return { query: asText(query), generatedAt: nowIso(options), vectorModel: VECTOR_MODEL, results, stats: { indexedItems: asArray(index?.items).length, returnedItems: results.length } };
  }
  ...
  global.AHASemanticRetrieval = { STORAGE_KEY, VERSION, VECTOR_MODEL, extractSemanticFeatures, buildLocalVector, cosineSimilarity, semanticSimilarity, buildSemanticIndex, saveSemanticIndex, loadSemanticIndex, refreshSemanticIndex, getSemanticStatus, searchSemanticKnowledge, hybridSearch, buildSemanticRagContext, buildSemanticPromptBlock };
