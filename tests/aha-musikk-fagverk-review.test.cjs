...
const runtimeCorpus=JSON.parse(fs.readFileSync("data/integrations/runtime/history-go-fagverk-musikk.corpus.v1.json","utf8"));
const runtimePolicy=JSON.parse(fs.readFileSync("data/integrations/runtime/history-go-fagverk-musikk.policy.v1.json","utf8"));
...
assert.equal(activeMusikk.policy_path,"data/integrations/runtime/history-go-fagverk-musikk.policy.v1.json");
assert.equal(runtimeCorpus.projection_mode,"reviewed_anchor_projection_v1");
assert.equal(runtimeCorpus.chapter_count,8);
...
assert.equal(Object.keys(runtimePolicy.chapter_rules).length,8);
...
console.log(`Musikk review/runtime gate passed: ${evaluation.summary.passed}/${evaluation.summary.total} constructed cases, ${fixtures.summary.passed}/${fixtures.summary.total} fixtures, 8 runtime chapters.`);
