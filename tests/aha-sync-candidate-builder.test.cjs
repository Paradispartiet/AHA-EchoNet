...
assert.equal(questionCandidate.approvalBoundary, 'personal_ai_loop_source_approval');
assert.equal(questionCandidate.approvalState, 'suggested');
assert.equal(questionCandidate.approved, undefined, 'candidate must not be approved automatically');
...
assert.equal(fs.existsSync(path.join(root, 'js/ahaSyncConfirmationGate.js')), false, 'must not create a parallel sync confirmation gate');
assert.equal(flatCandidates.every((candidate) => candidate.approvalBoundary === 'personal_ai_loop_source_approval'), true, 'all candidates must point to the existing source approval boundary');
assert.equal(flatCandidates.every((candidate) => candidate.approvalState === 'suggested'), true, 'all candidates must start suggested');
assert.equal(flatCandidates.some((candidate) => candidate.approved === true || candidate.approvalState === 'approved'), false, 'no candidate may be approved automatically');
